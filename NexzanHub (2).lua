...
function Controls.Slider(parent, cfg)
	...
	-- Drag slider memakai pola arbitrasi input resmi WindUI
	-- (WindUI.CurrentInput + GUID), sama seperti Slider bawaan WindUI.
	-- Selama slider ditarik, drag Window otomatis tidak ikut jalan.
	local dragging = false
	local activeInput = nil
	local CurInput = WindUI.GenerateGUID and WindUI.GenerateGUID() or tostring({})
	...
	Util.Bin.Connect(hit.InputBegan, function(input)
		...
		-- Input sedang dipakai komponen WindUI lain (drag window / resize / slider lain)
...
local function buildWatermark(ctx)
	...
	-- Drag memakai Creator.Drag milik WindUI (BUKAN implementasi sendiri).
	-- Dengan begitu watermark ikut sistem arbitrasi input WindUI.CurrentInput,
	-- sehingga tidak pernah bertabrakan dengan drag/resize Window.
	...
	if type(Creator.Drag) == "function" then
		Watermark.DragModule = Creator.Drag(wmFrame, { hit }, function(dragging)
			Kit.Anim(wmFrame, 0.12, { ImageTransparency = dragging and 0 or 0.05 })
		end)
	else
		-- Build WindUI yang sangat lama tidak punya Creator.Drag:
		-- watermark tetap tampil, hanya tidak bisa digeser (aman, tanpa error).
		hit.Active = false
	end
end
...
function WindowExtra.Init(ctx)
	...
	--// ---- Snap: dijalankan setelah drag WindUI selesai ----
	-- Tidak ada handler input sendiri. Kita hanya MENGAMATI Window.Dragging
	-- (di-set oleh callback drag bawaan WindUI), lalu merapikan posisi.
	...
	--// ---- Resize ----
	-- WindUI SUDAH punya ResizeHandle sendiri di sudut kanan-bawah lengkap
	-- dengan arbitrasi WindUI.CurrentInput. Membuat grip sendiri hanya akan
	-- bertabrakan, jadi kita cukup memakai milik WindUI dan menyimpan hasilnya.
	-- Window:IsResizable(v) bawaan tetap berfungsi seperti biasa.
	...
	-- Listener ini PASIF: hanya membaca posisi klik, tidak pernah meng-klaim
	-- WindUI.CurrentInput, sehingga drag/resize bawaan tidak terganggu.
	Util.Bin.Connect(UserInputService.InputBegan, function(input)
		...
		-- Sedang drag / resize window (input dikuasai WindUI) -> abaikan.
		if Window.Dragging or WindUI.CurrentInput ~= nil then
			return
		end
		task.defer(function()
