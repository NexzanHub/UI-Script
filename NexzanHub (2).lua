...
local DefaultOptions = {
	...
	RememberWindow = true, -- ingat posisi & ukuran window terakhir
...
function Controls.Slider(parent, cfg)
	...
	-- Slider TANPA drag: memakai event lokal GuiObject saja
	-- (InputBegan/InputChanged/InputEnded milik hit-area itu sendiri).
	-- Tidak ada listener UserInputService global dan tidak menyentuh
	-- state input WindUI, sehingga drag/resize bawaan tidak terganggu.
	local activeInput = nil

	local function fromPosition(position)
		local relative = (position.X - bar.AbsolutePosition.X) / math.max(1, bar.AbsoluteSize.X)
	...
	-- InputChanged pada hit-area: hanya aktif selama pointer menekan slider.
	Util.Bin.Connect(hit.InputChanged, function(input)
	...
	Util.Bin.Connect(hit.InputEnded, function(input)
		if
			input.UserInputType == Enum.UserInputType.MouseButton1
			or input.UserInputType == Enum.UserInputType.Touch
		then
	...
	-- Pointer keluar dari area slider -> hentikan (pengganti listener global).
	Util.Bin.Connect(hit.MouseLeave, stop)
...
-- Preset sudut layar; pengganti drag (posisi statis, tanpa handler input).
local WM_CORNERS = {
...
local function applyCorner()
	...
	local preset = WM_CORNERS[Watermark.Corner] or WM_CORNERS.TopLeft
	wmFrame.Position = preset[1]
	wmFrame.AnchorPoint = preset[2]
end
...
local function buildWatermark(ctx)
	...
	-- TIDAK ADA DRAG.
	-- Extension ini sengaja tidak memiliki sistem drag sama sekali —
	-- drag adalah milik WindUI sepenuhnya. Watermark bersifat statis dan
	-- diposisikan lewat preset sudut (Nexzan:SetWatermarkPosition).
	-- Tidak ada hit-area / handler input, sehingga tidak mungkin bertabrakan.
end
...
function Watermark.Init(ctx)
	...
	function Watermark.SetCorner(corner)
		if WM_CORNERS[corner] then
			Watermark.Corner = corner
			applyCorner()
		end
		return Watermark.Corner
	end
...
--//      Remember Position/Size + Auto Save + Keyboard Shortcut.
--//
--//      CATATAN PENTING:
--//      Extension ini TIDAK memiliki sistem drag/resize/snap sama sekali.
--//      Drag & resize sepenuhnya milik WindUI.
--// ==========================================================================
...
function WindowExtra.Init(ctx)
	...
	--// ---- Drag & Resize: TIDAK ADA DI EXTENSION INI ----
	-- WindUI sudah punya sistem drag dan resize handle sendiri.
	-- Extension TIDAK membuat drag, resize, maupun snap apa pun, dan tidak
	-- pernah mengubah Position/Size window — sehingga mustahil bertabrakan.
	-- Window:IsResizable(v) dan drag topbar bawaan berfungsi 100% seperti asli.
	...
	-- Listener PASIF: hanya membaca posisi klik untuk menutup panel.
	-- Tidak memindahkan apa pun, tidak menyentuh state input WindUI.
	Util.Bin.Connect(UserInputService.InputBegan, function(input)
		...
		-- Window sedang di-drag oleh WindUI -> jangan tutup panel.
...
--- Pengganti drag watermark: pilih sudut layar.
--- corner = "TopLeft" | "TopRight" | "BottomLeft" | "BottomRight"
function Nexzan:SetWatermarkPosition(corner)
	if Nexzan.Watermark and Nexzan.Watermark.SetCorner then
		return Nexzan.Watermark.SetCorner(corner)
	end
	return nil
end
