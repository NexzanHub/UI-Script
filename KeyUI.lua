...
--- Ubah berbagai bentuk sumber gambar menjadi asset yang bisa dipakai Roblox.
--- Mendukung:
---   "rbxassetid://123"          -> langsung
---   "123"                       -> rbxassetid://123
---   "https://..../gambar.png"   -> diunduh, lalu getcustomasset
--- Return: assetString atau nil + alasan gagal.
local imageCache = {}

function Util.ResolveImage(source)
	if type(source) ~= "string" or source == "" then
		return nil, "Sumber gambar kosong"
	end

	-- sudah berupa asset Roblox
	if source:match("^rbxassetid://%d+") then
		return source
	end
	if source:match("^rbxthumb://") or source:match("^rbxgameasset://") then
		return source
	end
	...
	if source:match("^https?://") then
		if imageCache[source] then
			return imageCache[source]
		end

		if not Util.File.Enabled then
			return nil, "Executor tidak punya filesystem"
		end
		if not getcustomasset then
			return nil, "Executor tidak mendukung getcustomasset"
		end

		-- tentukan ekstensi dari URL (buang query string)
		...
		-- nama file aman & unik per URL
		local safe = source:gsub("[^%w]", "_")
		if #safe > 80 then
			safe = safe:sub(1, 40) .. "_" .. tostring(#source) .. safe:sub(-20)
		end

		local folder = KEYUI_FOLDER .. "/assets"
		Util.File.Ensure(KEYUI_FOLDER)
		Util.File.Ensure(folder)

		local path = folder .. "/" .. safe .. "." .. ext

		if not isfile(path) then
			local body = Util.HttpGet(source)
			if not body or body == "" then
				return nil, "Gagal mengunduh gambar"
			end
			local okWrite = Util.File.Write(path, body)
			if not okWrite or not isfile(path) then
				return nil, "Gagal menyimpan gambar"
			end
		end

		local ok, asset = pcall(getcustomasset, path)
		if not ok or not asset then
			return nil, "getcustomasset gagal"
		end

		imageCache[source] = asset
		return asset
	end

	return nil, "Format sumber gambar tidak dikenali"
end

--- Kosongkan cache gambar (dipakai saat Destroy).
function Util.ClearImageCache()
...
function Icons.Get(name)
	...
	-- Dukungan langsung rbxassetid:// dan angka
	if name:match("^rbxassetid://%d+") or name:match("^rbxthumb://") then
	...
	-- Raw URL: diunduh lalu dipakai lewat getcustomasset
	if name:match("^https?://") then
		if Icons.cache[name] ~= nil then
			return Icons.cache[name] or nil
		end
		local asset = Util.ResolveImage(name)
		if asset then
			local data = { Image = asset, ImageRectPosition = Vector2.zero, ImageRectSize = Vector2.zero }
...
function KeyWindow:BuildThumbnail()
	...
	local isTable = type(thumb) == "table"

	local height = (isTable and thumb.Height) or 150
	local source = (isTable and thumb.Image) or thumb
	local scaleType = (isTable and thumb.ScaleType) or "Crop"
	local corner = (isTable and thumb.Corner) or 14
	...
	local imageLabel = Kit.New("ImageLabel", {
		...
		Image = "",
		ImageTransparency = 1,
		ScaleType = Enum.ScaleType[scaleType] or Enum.ScaleType.Crop,
		ZIndex = 13,
	}, {
		Kit.New("UICorner", { CornerRadius = UDim.new(0, corner) }),
	})

	-- placeholder saat gambar sedang diunduh / gagal
	local placeholder = Kit.Shape(corner, "Squircle", {
		Size = UDim2.new(1, 0, 1, 0),
		ImageTransparency = 0.94,
		ThemeTag = { ImageColor3 = "Text" },
		ZIndex = 12,
	...
	self.Thumbnail = Kit.New("Frame", {
		...
		placeholder,
		imageLabel,
		Kit.Outline(corner, { ImageTransparency = 0.9, ZIndex = 14 }),
	})

	self.ThumbnailImage = imageLabel

	--// Resolve gambar. URL eksternal butuh unduh, jadi dijalankan async
	--// supaya jendela tidak tertahan menunggu jaringan.
	local function applyAsset(asset)
		if not imageLabel.Parent then
			return
		end
		imageLabel.Image = asset
		Kit.Tween(imageLabel, 0.28, { ImageTransparency = 0 })
		Kit.Tween(placeholder, 0.28, { ImageTransparency = 1 })
	end

	local sourceStr = tostring(source or "")
	local isRemote = sourceStr:match("^https?://") ~= nil

	if not isRemote then
		-- rbxassetid / angka: langsung, tanpa jeda
		local asset, reason = Util.ResolveImage(sourceStr)
		if asset then
			applyAsset(asset)
		else
			warn(("[ %s ] Thumbnail: %s"):format(KEYUI_NAME, tostring(reason)))
		end
		return
	end
	...
	task.spawn(function()
		...
		if asset then
			...
			warn(("[ %s ] Thumbnail gagal dimuat: %s"):format(KEYUI_NAME, tostring(reason)))
			if cfg.OnThumbnailFail then
				pcall(cfg.OnThumbnailFail, reason)
			end
		end
	end)
end
